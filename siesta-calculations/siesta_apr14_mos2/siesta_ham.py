...
hsx = sisl.io.siesta.hsxSileSiesta("MoS2.HSX")
...
for n in range(48):
    print(np.conj(ckx[:, n]).T @ Skx @ ckx[:, n])

"""print(np.conj(ck[:, 0]).T @ Sk @ ckx[:, 0])

phase_y = np.conj(ck[:, 20]).T @ Sk @ ckx[:, 20]
phase_y = phase_y/np.abs(phase_y)
print("phase: " + str(phase_y))

for i in range(0, 50):
    print(abs(np.conj(ck[:, i]).T @ Sk @ ckx[:, i]))
"""
