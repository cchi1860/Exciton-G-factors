...
Skxinv = np.linalg.inv(Skx)
Skyinv = np.linalg.inv(Sky)

Ek, ck = eig(Skinv @ Hk)
"""print([f"{(x-3.621):.3f}" for x in Ek])
#eigenvalues might have an offset equal to the fermi level in the .dat file"""
Ekx, ckx = eig(Skxinv @ Hk)
Eky, cky = eig(Skyinv @ Hk)


idx = np.argsort(Ek.real) 
Ek = Ek[idx]
ck = ck[idx]

idxx = np.argsort(Ekx.real) 
Ekx = Ekx[idx]
ckx = ckx[idx]

idxy = np.argsort(Eky.real) 
Eky = Eky[idx]
cky = cky[idx]

"""# checking generalized inner product
for n in range(48):
    print(np.conj(ck[:, n]).T @ ck[:, n])"""


for n in range (44, 48):
    phase_x = np.conj(ck[:, n]).T @ ckx[:, n]
    phase_y = np.conj(ck[:, n]).T @ cky[:, n]
    ...
    berry_1 = np.conj(du_dkx).T @ du_dky
    berry_2 = np.conj(du_dky).T @ du_dkx  
    print("Berry curvature of state:", n, -1j*(berry_1 - berry_2)*0.52918**2)

    rhs_x = Hk @ du_dkx
    rhs_x2 = Ek[n] * (du_dkx) # probably need to include this in the generalized case?
